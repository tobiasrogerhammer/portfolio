...
export const StatisticsDashboard = ({ skills }: { skills: Skill[] }) => {
  ...
  const totalTechnologies = technologies.reduce((sum, skill) => sum + skill.items.length, 0)
  ...
  return (
    <div className="mt-12 sm:mt-6">
      ...
      <div className="mb-10">
        ...
        {/* Desktop: Simplified cards (single column on tablet, row of 3 on desktop) */}
            ...
            return (
              ...
              >
                <CardContent className="p-4 sm:p-5 h-full">
                  <div className="flex items-start gap-4 lg:flex-col lg:gap-3 lg:h-full">
                    {/* Header block */}
                    <div className="w-36 flex-shrink-0 lg:w-full">
                    ...
                    {/* Separator (vertical on tablet, horizontal on desktop) */}
                    <div className="h-10 w-px bg-border/80 mt-1 lg:h-px lg:w-full lg:mt-0" aria-hidden />

                    {/* Skills tags */}
                    <div className="flex-1 flex flex-wrap gap-1.5 sm:gap-2 lg:flex-none lg:w-full">
                      {tech.items.map((item) => (
                        <span
                          key={item}
